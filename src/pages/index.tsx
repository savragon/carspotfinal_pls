...
export default function App() {
  ...
  const [selectedTab, setSelectedTab] = useState<
    "home" | "carcast" | "map" | "settings" | "profile" | "policies"
  >("carcast");
  ...
  const fetchData = async () => {
    if (cityName) {
      ...
      const forecastUrl = `https://api.openweathermap.org/data/2.5/forecast?q=${cityName}&appid=3c0328747012edae286e57d70ce918c5&units=metric`;
  ...
  return (
    <div className="bg-custom h-screen w-full bg-cover bg-center bg-no-repeat font-medium">
      <FaBars
        className="fixed left-5 top-5 text-3xl text-white"
        onClick={() => setIsOpen(true)}
      />
      {isOpen && (
        <div className="option-menu fixed flex h-full w-full flex-col bg-stone-100 p-4">
          <div
            className="bar absolute left-0 top-0 cursor-pointer p-5"
            onClick={toggleMenu}
          >
            <FaTimes className="text-4xl text-gray-700" />
          </div>
          <div className="flex flex-col items-center justify-center p-20">
            <a href="./home">
              <p className="p-10 text-5xl font-medium text-gray-700 duration-300 hover:text-gray-500">
                Home
              </p>
            </a>
            <a href="./map">
      ...
      {!isOpen && ( // Add a conditional check here
        <div className="items-center justify-center bg-blue-400 p-5 text-white">
